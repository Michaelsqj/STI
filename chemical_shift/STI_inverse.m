function [chi,chemshift,flag,relres,iter,resvec,lsvec] = STI_inverse(STIParams_filename, maxit, tol, alpha)
    % Input:  
    ...
    chemshift = reshape(chi_tensor(9*VoxNum+1: 10*VoxNum), sizeVol(1),sizeVol(2),sizeVol(3));
    ...
    function y = afun(x,transp_flag)
        ...
        if strcmp(transp_flag,'transp')              % y = A'*x
            y = zeros(10*VoxNum, 1, 'single');          % chitensor (10 VoxNum X 1), one for chemical shift
            ...
            for orient_i = 1:OriNum
                ...
                y(1:9*VoxNum) = y(1:9*VoxNum) + [chi_temp{1,1}(:);chi_temp{1,2}(:);chi_temp{1,3}(:);...
            ...
            y(9*VoxNum+1:10*VoxNum)=sum(delta, 2);
            ...
            y(1:9*VoxNum) = y(1:9*VoxNum) + alpha*delta.*mask_arr;
        ...
        elseif strcmp(transp_flag,'notransp')            % y = A*x
                ...
                Fc = reshape(x(9*VoxNum+1: 10*VoxNum), sizeVol(1), sizeVol(2), sizeVol(3));            
                ...
                for orient_i = 1:OriNum
                        ...
                        y(((orient_i - 1)*VoxNum+1) : orient_i*VoxNum) = delta(:) + Fc(:);          
                end
                ...
                y(OriNum*VoxNum+1:(OriNum+9)*VoxNum) = alpha*x(1:9*VoxNum).*cat(1, ~BrainMask(:), ~STIParams.maskMSA(:), ~STIParams.maskMSA(:),...
                                                                          ~STIParams.maskMSA(:), ~BrainMask(:), ~STIParams.maskMSA(:),...
                                                                          ~STIParams.maskMSA(:), ~STIParams.maskMSA(:), ~BrainMask(:));
